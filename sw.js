  ...
  if (action === 'close') {
    ...
    event.waitUntil(
      clients.matchAll().then(clis => {
        ...
        if (client !== undefined) {
          client.navigate('/samples/notified.html?navigate');
          client.focus();
        } else {
          // there are no visible windows. Open one.
          clients.openWindow('/samples/notified.html?opend');
          notification.close();
        }
      })
    );
  ...
  event.waitUntil(
    clients.matchAll().then(c => {
      ...
      if (c.length === 0) {
        ...
        return self.registration.showNotification('Already open', options);
      }
    })
  );
});
